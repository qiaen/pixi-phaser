...
- 可用素材：
  ...
  - 卡片与 UI：`/images/Card/`、`/images/interface/`（Sun.gif、Shovel/0.gif 铲子、ZombiesWon.png、Almanac_*、FinalWave.gif 等）。
  - 音频：项目内只有 `/zw/bg.mp3`（背景音乐）和 `/zw/bz.mp3`（爆炸音效），无其它音效素材。
- 新版游戏页面路由：`/zwdzjs` → `src/views/zwdzjs/`
  ...
  - 变异规则：草坪左侧 `SAFE_COLS`（默认 2）列是「家门口」安全区（舞台上画有虚线安全线），种在安全区之外的植物有 `TURN_ZOMBIE_RATE`（默认 40%）概率当场变成僵尸（原地冒出僵尸 + `ZombieHand.png` 特效）。
